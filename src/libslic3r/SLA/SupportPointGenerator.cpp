    ...
    std::function<void(void)> throw_on_cancel)
{
                      ...
                      [&layers, &heights, throw_on_cancel] (size_t layer_id)
    {
      ...
      const float safe_angle = 25.f * (float(M_PI)/180.f); // smaller number - less supports
...
{
    // TODO: select each type of surface (overrhang, dangling, slope), derive the support force deficit for it and call uniformly conver with the right params
    ...
    if (! s.dangling_areas.empty()) {
        ...
        uniformly_cover(s.dangling_areas, s, a * tp - current / 5.f, grid3d, false, false);
    }

    if (! s.overhangs_slopes.empty()) {
        double a = std::accumulate(s.overhangs_slopes.begin(), s.overhangs_slopes.end(), 0., areafn);
        uniformly_cover(s.overhangs_slopes, s, s.zlevel * a * tp - current / 50.f, grid3d, false, false);
    }
